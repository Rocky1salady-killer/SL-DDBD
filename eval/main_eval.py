...
def validate(config, data_loader, model):
    ...
    labellist=torch.zeros(0,dtype=torch.long, device='cuda')
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        labellist=torch.cat([labellist,target.view(-1).cuda()])
    ...
    ans = ''
    acc_each_class = [pre_num_meter[i] / cla_num_meter[i] if cla_num_meter[i] != 0 else 0 for i in range(config.MODEL.NUM_CLASSES)]
...
# Confusion matrix
    ...
    for i in range(len(labellist)):
        confusion_matrix[labellist[i], predlist[i]] += 1
...
# Plotting the confusion matrix
    ...
    plt.colorbar()
    tick_marks = np.arange(config.MODEL.NUM_CLASSES)
    plt.xticks(tick_marks, config.DATA.NAME_CLASSES, rotation=45)
    plt.yticks(tick_marks, config.DATA.NAME_CLASSES)
    ...
    plt.savefig('confusion matrix/confusion_matrix.png')  # Save the figure to a file
